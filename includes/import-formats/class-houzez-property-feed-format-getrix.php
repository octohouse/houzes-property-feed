...
class Houzez_Property_Feed_Format_Getrix extends Houzez_Property_Feed_Process {
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_getrix", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_getrix", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
			...
			update_option( 'houzez_property_feed_property_' . $this->import_id, (string)$property_attributes['IDImmobile'], false );
			
			$this->log( 'Importing property ' . $property_row . ' with reference ' . (string)$property_attributes['IDImmobile'], (string)$property_attributes['IDImmobile'], 0, '', false );
			...
			$args = array(
	            ...
	            'meta_query' => array(
	            	array(
		            	...
		            	'value' => (string)$property_attributes['IDImmobile']
		            )
	            )
	        );
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', (string)$property_attributes['IDImmobile'], $post_id );
					...
					{
						$this->log_error( 'Failed to update post. The error was as follows: ' . $post_id->get_error_message(), (string)$property_attributes['IDImmobile'] );
					}
					else
					{
	        ...
	        {
	        	$this->log( 'This property hasn\'t been imported before. Inserting it', (string)$property_attributes['IDImmobile'] );
				...
				if ( is_wp_error( $post_id ) ) 
				{
					$this->log_error( 'Failed to insert post. The error was as follows: ' . $post_id->get_error_message(), (string)$property_attributes['IDImmobile'] );
				}
				else
				{
			...
			if ( $inserted_updated !== false )
			{
				...
				$this->log( 'Successfully ' . $inserted_updated . ' post', (string)$property_attributes['IDImmobile'], $post_id );
                ...
                {
                	$price = '';
					if ( isset($property->Prezzo) && (string)$property->Prezzo != '' )
					{
						if ( is_numeric((string)$property->Prezzo) )
						{
                ...
                update_post_meta( $post_id, 'fave_property_bedrooms', ( ( isset($property->Residenziale->NrCamereLetto) ) ? (string)$property->Residenziale->NrCamereLetto : '' ) );
	            update_post_meta( $post_id, 'fave_property_bathrooms', ( ( isset($property->Residenziale->NrBagni) ) ? (string)$property->Residenziale->NrBagni : '' ) );
	            update_post_meta( $post_id, 'fave_property_rooms', '' );
	            
	            $area = ( ( isset($property->MQSuperficie) ) ? (string)$property->MQSuperficie : '' );
				...
				if ( isset($property->Zona) && (string)$property->Zona != '' )
				{
					$address_parts[] = (string)$property->Zona;
				}
				if ( isset($property->Comune) && (string)$property->Comune != '' )
				{
					$address_parts[] = (string)$property->Comune;
	            ...
	            $lat = '';
	            $lng = '';
	            if ( isset($property->Latitudine) && !empty((string)$property->Latitudine) )
	            {
	                update_post_meta( $post_id, 'houzez_geolocation_lat', (string)$property->Latitudine );
	                $lat = (string)$property->Latitudine;
	            }
	            if ( isset($property->Longitudine) && !empty((string)$property->Longitudine) )
	            {
	                update_post_meta( $post_id, 'houzez_geolocation_long', (string)$property->Longitudine );
	                $lng = (string)$property->Longitudine;
	            }
				...
				{
					if ( isset($taxonomy_mappings[(string)$property->Tipologia]) && !empty($taxonomy_mappings[(string)$property->Tipologia]) )
					{
						wp_set_object_terms( $post_id, (int)$taxonomy_mappings[(string)$property->Tipologia], "property_type" );
					}
					else
					{
						$this->log( 'Received property type of ' . (string)$property->Tipologia . ' that isn\'t mapped in the import settings', (string)$property_attributes['IDImmobile'], $post_id );

						$import_settings = $this->add_missing_mapping( $mappings, 'property_type', (string)$property->Tipologia, $this->import_id );
					}
				}
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_getrix', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
					...
					$this->log( 'Imported ' . count($urls) . ' photo URLs', (string)$property_attributes['IDImmobile'], $post_id );
				}
				else
				{
					$media_ids = array();
					...
					if ( !empty($previous_import_media_ids) )
					{
						...
						if ( $explode_previous_import_media_ids[0] == $post_id )
						{
							...
							if ( isset($explode_previous_import_media_ids[1]) && !empty($explode_previous_import_media_ids[1]) )
							{
								...
								$this->log( 'Imported ' . count($media_ids) . ' images before failing in the previous import. Continuing from here', (string)$property_attributes['IDImmobile'], $post_id );
							}
						}
					}
                        	...
                        	if ( isset($immagini->Immagine) )
			                {
		                        foreach ($immagini->Immagine as $immagine)
		                        {
									...
									)
									{
										...
										$description = (string)$immagine->Titolo;
										$modified = (string)$immagine->DataModifica;
										...
										{
											if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, (string)$property_attributes['IDImmobile'], $url, $url, $description, 'image', $image_i, date("Y-m-d H:i:s", strtotime($modified)) ) === true )
											{
												$this->ping();
											    ...
											    if ( is_wp_error( $tmp ) ) 
											    {
											        $this->log_error( 'An error occurred whilst importing ' . $url . '. The error was as follows: ' . $tmp->get_error_message(), (string)$property_attributes['IDImmobile'], $post_id );
											    }
											    else
											    {
												    ...
												    if ( is_wp_error( $id ) ) 
												    {
												        ...
												        $this->log_error( 'ERROR: An error occurred whilst importing ' . $url . '. The error was as follows: ' . $id->get_error_message(), (string)$property_attributes['IDImmobile'], $post_id );
												    }
												    else
												    {
												    	$media_ids[] = $id;
					...
					$this->log( 'Imported ' . count($media_ids) . ' photos (' . $new . ' new, ' . $existing . ' existing, ' . $deleted . ' deleted)', (string)$property_attributes['IDImmobile'], $post_id );
					if ( $queued > 0 ) 
					{
						$this->log( $queued . ' photos added to download queue', (string)$property_attributes['IDImmobile'], $post_id );
					}
				...
				do_action( "houzez_property_feed_property_imported_getrix", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_getrix", $this->import_id );
