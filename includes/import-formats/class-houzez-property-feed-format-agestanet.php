...
class Houzez_Property_Feed_Format_Agestanet extends Houzez_Property_Feed_Process {
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_agestanet", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_agestanet", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $my_post = array(
				    	...
				    	'post_excerpt'   => (string)$property->testo,
				    	'post_content' 	 => (string)$property->testo,
				    	'post_status'    => 'publish',
				  	);
			...
			if ( $inserted_updated !== false )
			{
	            ...
	            update_post_meta( $post_id, 'fave_property_bathrooms', ( ( isset($property->bagni) ) ? (string)$property->bagni : '' ) );
	            update_post_meta( $post_id, 'fave_property_rooms', '' );
	            
	            update_post_meta( $post_id, 'fave_property_size', ( ( isset($property->mq) && !empty((string)$property->mq) ) ? (string)$property->mq : '' ) );
	            update_post_meta( $post_id, 'fave_property_size_prefix', ( ( isset($property->mq) && !empty((string)$property->mq) ) ? 'Sq M' : '' ) );

	            update_post_meta( $post_id, 'fave_property_garage', '' );
	            update_post_meta( $post_id, 'fave_property_id', (string)$property->rif );

	            $address_parts = array();
	            if ( isset($property->comune) && (string)$property->comune != '' )
	            {
	                $address_parts[] = ucwords(strtolower((string)$property->comune));
	            }
	            if ( isset($property->provincia) && (string)$property->provincia != '' )
	            {
	                $address_parts[] = ucwords(strtolower((string)$property->provincia));
	            }
	            if ( isset($property->regione) && (string)$property->regione != '' )
	            {
	                $address_parts[] = ucwords(strtolower((string)$property->regione));
	            }
	            $address_parts = array_unique($address_parts);

	            update_post_meta( $post_id, 'fave_property_map', (string)$property->mappa_visibile );
		            	...
		            	case "author_info":
		            	{
		            		foreach ( $import_settings['agent_display_option_rules'] as $rule )
		            		{
		            			...
		            			switch ( $rule['field'] )
		            			{
		            				default:
		            				{
		            					$value_in_feed_to_check = isset($this->{$rule['field']}) ? (string)$this->{$rule['field']} : '';
		            				}
		            			}
				...
				if ( isset($property->tipologia) )
				{
					...
					{
						$this->log( 'Received property type of ' . (string)$property->tipologia . ' that isn\'t mapped in the import settings', (string)$property_attributes['id'], $post_id );
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_agestanet', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
				...
				do_action( "houzez_property_feed_property_imported_agestanet", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_agestanet", $this->import_id );
