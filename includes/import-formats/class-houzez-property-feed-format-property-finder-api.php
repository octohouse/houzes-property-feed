<?php
/**
 * Class for managing the import process of a Property Finder JSON file
 *
 * @package WordPress
 */
if ( class_exists( 'Houzez_Property_Feed_Process' ) ) {

class Houzez_Property_Feed_Format_Property_Finder_API extends Houzez_Property_Feed_Process {
	...
	private function get_token( $test = false )
    {
        ...
        $response = wp_remote_post(
            ...
            array(
                ...
                'body' => json_encode(array( 
                    'apiKey' => ( ( isset($import_settings['api_key']) ) ? $import_settings['api_key'] : '' ), 
                    'apiSecret' => ( ( isset($import_settings['api_secret']) ) ? $import_settings['api_secret'] : '' ) 
                )),
            )
        );
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    $this->log_error( 'Failed to get accessToken part of response body: ' . $response['body'] );
                    return false;
                }
            }
        }
	...
	public function parse( $test = false )
	{
        ...
        while ( $more_properties )
        {
            ...
            if ( isset($json['errors']) && !empty($json['errors']) )
            {
                ...
                {
                    $this->log_error( 'Error returned by Property Finder: ' . print_r($error, TRUE) );
                }
                return false;
            }
            
            $total_pages = '';
            if ( isset($json['pagination']['totalPages']) )
            {
                if ( $current_page == $json['pagination']['totalPages'] )
                {
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_property_finder_api", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_property_finder_api", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
	        ...
	        $display_address = $property['title']['en'];

			$post_content = $post_excerpt = isset($property['description']['en']) ? $property['description']['en'] : '';
			...
			if ( $inserted_updated !== false )
			{
				...
				$department = $property['price']['type'] == 'sale' ? 'residential-sales' : 'residential-lettings';
                ...
                else
                {
                	...
                	{
                		$price = preg_replace("/[^0-9.]/", '', $property['price']['amounts']['sale']);
	                    if ( !empty($price) )
	                    {
	                        $price = round($price);
	                    }
	                ...
	                elseif ( $department == 'residential-lettings' )
	                {
	                    ...
	                    if ( isset($property['price']['daily']) && !empty($property['price']['daily']) )
	                    {
	                        $rent = preg_replace("/[^0-9.]/", '', $property['price']['daily']);
	                        if ( !empty($rent) )
	                        {
	                            $rent = round($rent);
	                            $rent_frequency = 'pd';
	                    ...
	                    elseif ( isset($property['price']['monthly']) && !empty($property['price']['monthly']) )
	                    {
	                        $rent = preg_replace("/[^0-9.]/", '', $property['price']['monthly']);
	                        if ( !empty($rent) )
	                        {
	                            $rent = round($rent);
	                            $rent_frequency = 'pcm';
	                    ...
	                    elseif ( isset($property['price']['weekly']) && !empty($property['price']['weekly']) )
	                    {
	                        $rent = preg_replace("/[^0-9.]/", '', $property['price']['weekly']);
	                        if ( !empty($rent) )
	                        {
	                            $rent = round($rent);
	                            $rent_frequency = 'pw';
	                    ...
	                    elseif ( isset($property['price']['yearly']) && !empty($property['price']['yearly']) )
	                    {
	                        $rent = preg_replace("/[^0-9.]/", '', $property['price']['yearly']);
	                        if ( !empty($rent) )
	                        {
	                            $rent = round($rent);
	                            $rent_frequency = 'pa';
	            ...
	            update_post_meta( $post_id, 'fave_property_id', ( ( isset($property['reference']) ) ? $property['reference'] : '' ) );

	            $address_number = '';
				$address_property_finder_api = isset($property['address']['line_1']) ? $property['address']['line_1'] : '';
				$address1_explode = explode(' ', $address_property_finder_api);
				if ( !empty($address1_explode) && is_numeric($address1_explode[0]) )
				{
					$address_number = array_shift($address1_explode);
					$address_property_finder_api = implode(' ', $address1_explode);
				}
	            ...
	            if ( isset($property['location']['lon']) && !empty($property['location']['lon']) )
	            {
	                update_post_meta( $post_id, 'houzez_geolocation_long', $property['location']['lon'] );
	            ...
	            if ( isset($property['amenities']) && is_array($property['amenities']) )
				{
					...
					{
						$feature = trim(ucfirst(str_replace("-", " ", $feature)));
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_property_finder_api', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();

					if (isset($property['media']['images']) && !empty($property['media']['images']))
	                {
	                    foreach ($property['media']['images'] as $photo)
	                    {
							if ( $pro_active === true )
							{
								if ( 
							...
							$size = apply_filters( 'houzez_property_feed_property_finder_api_image_size', $default_size ); // thumbnail, small, medium, large, hero, full
                        	$url = ( isset($photo[$size]['url']) ) ? $photo[$size]['url'] : $photo[$default_size]['url'];
				...
				$virtual_tours = array();
                if ( isset($property['media']['videos']['default']) && !empty($property['media']['videos']['default']) )
                {
                    $virtual_tours[] = $property['media']['videos']['default'];
                }
                if ( isset($property['media']['videos']['view360']) && !empty($property['media']['videos']['view360']) )
                {
                    $virtual_tours[] = $property['media']['videos']['view360'];
                }
				...
				do_action( "houzez_property_feed_property_imported_property_finder_api", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_property_finder_api", $this->import_id );
