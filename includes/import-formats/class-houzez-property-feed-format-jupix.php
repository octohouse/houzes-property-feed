...
class Houzez_Property_Feed_Format_Jupix extends Houzez_Property_Feed_Process {
	...
	public function import()
	{
		...
		{
			$this->log( 'Importing property ' . $property_row . ' with reference ' . (string)$property->propertyID, (string)$property->propertyID );
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( isset($property->availability) && !empty((string)$property->availability) )
				{
					...
					{
						wp_set_object_terms( $post_id, $taxonomy_mappings[(string)$property->availability], "property_status" );
					}
					else
					{
				...
				if ( isset($property->propertyType) && isset($property->propertyStyle) )
				{
					...
					{
						wp_set_object_terms( $post_id, $taxonomy_mappings[$jupix_type], "property_type" );
					}
					else
					{
				...
				do_action( "houzez_property_feed_property_imported_jupix", $post_id, $propert, $this->import_id );
