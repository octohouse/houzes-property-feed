	...
	public function import()
	{
		...
		foreach ( $this->properties as $property )
		{
			...
			if ( $inserted_updated !== false )
			{
	                        ...
	                        if (!empty($images->image))
	                        {
	                            foreach ($images->image as $image)
	                            {
									...
									)
									{
										...
										{
											if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, (string)$property->propertyID, $url, $url, $description, 'image', $image_i, $modified ) === true )
											{
												$this->ping();
				...
				do_action( "houzez_property_feed_property_imported_jupix", $post_id, $property, $this->import_id, $this->instance_id );
