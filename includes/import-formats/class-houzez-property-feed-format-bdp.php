		...
		foreach ( $this->properties as $property )
		{
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_bdp', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
					...
					if (isset($property['images']) && !empty($property['images']))
	                {
	                    foreach ($property['images'] as $image)
	                    {
	                        ...
	                        )
	                        {
								...
								{
									if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, $property['property_id'], $url, $url, $description, 'image', $image_i, '' ) === true )
									{
										$this->ping();
				...
				do_action( "houzez_property_feed_property_imported_bdp", $post_id, $property, $this->import_id, $this->instance_id );
