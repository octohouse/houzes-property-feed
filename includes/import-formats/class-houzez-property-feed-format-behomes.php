<?php
/**
 * Class for managing the import process of a Behomes JSON file
 *
 * @package WordPress
 */
if ( class_exists( 'Houzez_Property_Feed_Process' ) ) {

class Houzez_Property_Feed_Format_Behomes extends Houzez_Property_Feed_Process {
	...
	public function parse()
	{
		...
		$developers = isset($import_settings['developer']) && !empty($import_settings['developer']) ? explode(",", $import_settings['developer']) : array();
		$developers = array_map('trim', $developers);
		$developers = array_map('strtolower', $developers);
        $developers = array_filter($developers);

        $property_type = isset($import_settings['property_type']) && !empty($import_settings['property_type']) ? $import_settings['property_type'] : '';
		...
		while ( $more_properties )
		{
			...
			$url = 'https://api.behomes.tech/v3/get_behomes_objects?api_key=' . $import_settings['api_key'] . '&object_status=active&filter=yes&objects_per_page=' . $per_page . '&page=' . $current_page;
			/*if ( isset($import_settings['language']) && !empty($import_settings['language']) )
			{
				$url .= '&lang=' . strtolower(substr($import_settings['language'], 0, 2));
			}*/
			...
			if ($json !== FALSE)
			{
				...
				{
					if ( $current_page == $json['filter_params']['max_pages'] )
					{
				...
				{
					foreach ($json['response'] as $property)
					{
						if ( empty($developers) || ( !empty($developers) && isset($property['Organisation']['organizationName']) && in_array(strtolower($property['Organisation']['organizationName']), $developers) ) ) 
						{
							if ( empty($property_type) || ( !empty($property_type) && isset($property['OffplanOrSecondary']) && strtolower($property_type) == strtolower($property['OffplanOrSecondary']) ) )
							{
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_behomes", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_behomes", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
			if ( !empty($start_at_property) )
			{
				...
				if ( $property['project_id'] == $start_at_property )
				{
					// we found the property. We'll continue for this property onwards
					$this->log( 'Previous import failed to complete. Continuing from property ' . $property_row . ' with ID ' . $property['project_id'] );
			...
			update_option( 'houzez_property_feed_property_' . $this->import_id, $property['project_id'], false );
			
			$this->log( 'Importing property ' . $property_row . ' with reference ' . $property['project_id'], $property['project_id'], 0, '', false );
	        ...
	        $display_address = $property['ShortName'];
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', $property['project_id'], $post_id );
					...
					{
						$this->log_error( 'Failed to update post. The error was as follows: ' . $post_id->get_error_message(), $property['project_id'] );
					}
					else
					{
	        ...
	        {
	        	$this->log( 'This property hasn\'t been imported before. Inserting it', $property['project_id'] );
				...
				if ( is_wp_error( $post_id ) ) 
				{
					$this->log_error( 'Failed to insert post. The error was as follows: ' . $post_id->get_error_message(), $property['project_id'] );
				}
				else
				{
			...
			if ( $inserted_updated !== false )
			{
				...
				$this->log( 'Successfully ' . $inserted_updated . ' post', $property['project_id'], $post_id );

				update_post_meta( $post_id, $imported_ref_key, $property['project_id'] );
                ...
                else
                {
                	if ( $department == 'residential-sales' )
                	{
                		$price = '';
                		$prefix = '';
                		if ( isset($property['Layouts']) && !empty($property['Layouts']) )
                		{
                			if ( count($property['Layouts']) > 1 )
                			{
                				$prefix = 'From';
                			}
                			foreach ( $property['Layouts'] as $layout )
                			{
                				if ( 
                					isset($layout['OfferingType']['name']) && strtolower($layout['OfferingType']['name']) == 'sale' && 
                					isset($layout['layoutPrice100%']) && !empty($layout['layoutPrice100%']) 
                				)
                				{
                					if ( $price == '' || $layout['layoutPrice100%'] < $price)
                					{
                						$price = $layout['layoutPrice100%'];
                					}
                				}
                			}
                		}
	            ...
	            update_post_meta( $post_id, 'fave_property_id', $property['project_id'] );

	            update_post_meta( $post_id, 'fave_property_map', '1' );
	            update_post_meta( $post_id, 'fave_property_map_address', $property['Location']['address'] );
	            $lat = '';
	            $lng = '';
	            if ( isset($property['Location']['lat']) && !empty($property['Location']['lat']) )
	            {
	                update_post_meta( $post_id, 'houzez_geolocation_lat', $property['Location']['lat'] );
	                $lat = $property['Location']['lat'];
	            }
	            if ( isset($property['Location']['lng']) && !empty($property['Location']['lng']) )
	            {
	                update_post_meta( $post_id, 'houzez_geolocation_long', $property['Location']['lng'] );
		            	...
		            	case "author_info":
		            	{
		            		foreach ( $import_settings['agent_display_option_rules'] as $rule )
		            		{
		            			...
		            			switch ( $rule['field'] )
		            			{
		            				...
		            				{
		            					$value_in_feed_to_check = $property['Organisation']['organizationName'];
		            					break;
		            				}
		            			}
	            ...
	            if ( isset($property['Features']) && is_array($property['Features']) )
				{
					...
					{
						$term = term_exists( trim($feature['name_selected_language']), 'property_feature');
						...
						{
							$term = wp_insert_term( trim($feature['name_selected_language']), 'property_feature' );
				...
				if ( isset($property['attributes'][$status_field . '_status']) && !empty($property['attributes'][$status_field . '_status']) )
				{
					...
					{
						$this->log( 'Received status of ' . $property['attributes'][$status_field . '_status'] . ' that isn\'t mapped in the import settings', $property['project_id'], $post_id );
				...
				if ( isset($property['Layouts']) && !empty($property['Layouts']) )
        		{
        			...
        			{
        				if ( 
        					isset($layout['layoutType']['name_selected_language']) && !empty($layout['layoutType']['name_selected_language']) 
        				)
        				{
        					$property_types[] = $layout['layoutType']['name_selected_language'];
        				}
        			}
        		}
        		if ( !empty($property_types) )
        		{
	        		$counts = array_count_values($property_types);
					arsort($counts);
					array_keys($counts);
					$property_type = $counts[0];
				}
				...
				if ( !empty($property_type) )
				{
					...
					{
						$this->log( 'Received property type of ' . $property_type . ' that isn\'t mapped in the import settings', $property['project_id'], $post_id );
				...
				foreach ( $location_taxonomies as $location_taxonomy )
				{
					...
					if ( !empty($address_field_to_use) )
					{
						...
						if ( isset($property['Location'][$address_field_to_use]) && !empty($property['Location'][$address_field_to_use]) )
		            	{
		            		$term = term_exists( trim($property['Location'][$address_field_to_use]), $location_taxonomy);
							if ( $term !== 0 && $term !== null && isset($term['term_id']) )
							{
							...
							else
							{
								...
								{
									$term = wp_insert_term( trim($property['Location'][$address_field_to_use]), $location_taxonomy );
									if ( is_array($term) && isset($term['term_id']) )
									{
										$location_term_ids[] = (int)$term['term_id'];
									}
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_behomes', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
					...
					$this->log( 'Imported ' . count($urls) . ' photo URLs', $property['project_id'], $post_id );
				}
				else
				{
					$media_ids = array();
					...
					if ( !empty($previous_import_media_ids) )
					{
						...
						if ( $explode_previous_import_media_ids[0] == $post_id )
						{
							...
							if ( isset($explode_previous_import_media_ids[1]) && !empty($explode_previous_import_media_ids[1]) )
							{
								...
								$this->log( 'Imported ' . count($media_ids) . ' images before failing in the previous import. Continuing from here', $property['project_id'], $post_id );
							}
						}
					}
					...
					if (isset($property['Images']) && !empty($property['Images']))
					{
						foreach ($property['Images'] as $image)
						{
							...
							)
							{
								...
								{
									if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, $property['project_id'], $url, $url, $description, 'image', $image_i, '' ) === true )
									{
										$this->ping();
									    ...
									    if ( is_wp_error( $tmp ) ) 
									    {
									        $this->log_error( 'An error occurred whilst importing ' . $url . '. The error was as follows: ' . $tmp->get_error_message(), $property['project_id'], $post_id );
									    }
									    else
									    {
										    ...
										    if ( is_wp_error( $id ) ) 
										    {
										        ...
										        $this->log_error( 'ERROR: An error occurred whilst importing ' . $url . '. The error was as follows: ' . $id->get_error_message(), $property['project_id'], $post_id );
										    }
										    else
										    {
										    	$media_ids[] = $id;
					...
					$this->log( 'Imported ' . count($media_ids) . ' photos (' . $new . ' new, ' . $existing . ' existing, ' . $deleted . ' deleted)', $property['project_id'], $post_id );
					if ( $queued > 0 ) 
					{
						$this->log( $queued . ' photos added to download queue', $property['project_id'], $post_id );
					}
				...
				$virtual_tours = array();
				if ( isset($property['VideoPresentation']) && !empty($property['VideoPresentation']) )
				{
					$virtual_tours[] = $property['VideoPresentation'];
				}
				...
				// Layouts
				$fave_multi_units = array();
				if ( isset($property['Layouts']) && !empty($property['Layouts']) )
        		{
        			foreach ( $property['Layouts'] as $layout )
        			{
        				$unit = array(
        					'fave_mu_title' => ( isset($layout['layoutUnitNumber']) && !empty($layout['layoutUnitNumber']) ? $layout['layoutUnitNumber'] : '' ),
        					'fave_mu_price' => ( isset($layout['layoutPriceSelling']) && !empty($layout['layoutPriceSelling']) ? $layout['layoutPriceSelling'] : '' ),
        					'fave_mu_beds' => ( isset($layout['layoutBedRoomCount']) && !empty($layout['layoutBedRoomCount']) ? $layout['layoutBedRoomCount'] : '' ),
        					'fave_mu_baths' => ( isset($layout['layoutToiletCount']) && !empty($layout['layoutToiletCount']) ? $layout['layoutToiletCount'] : '' ),
        					'fave_mu_size' => ( isset($layout['layoutAreaInternal']) && !empty($layout['layoutAreaInternal']) ? $layout['layoutAreaInternal'] : '' ),
        					'fave_mu_size_postfix' => 'sq ' . $import_settings['unit'],
        					'fave_mu_type' => ( isset($layout['layoutType']['name_selected_language']) && !empty($layout['layoutType']['name_selected_language']) ? $layout['layoutType']['name_selected_language'] : '' ),
        					'fave_mu_availability_date' => ( isset($layout['layoutDeliveryDate']) && !empty($layout['layoutDeliveryDate']) ? date("d/m/Y", $layout['layoutDeliveryDate']/1000) : '' ),
        				);

        				$fave_multi_units[] = $unit;
        			}
        		}
        		update_post_meta( $post_id, 'fave_multi_units', $fave_multi_units );

				do_action( "houzez_property_feed_property_imported", $post_id, $property, $this->import_id, $this->instance_id );
				do_action( "houzez_property_feed_property_imported_behomes", $post_id, $property, $this->import_id, $this->instance_id );
				...
				if ( $inserted_updated == 'updated' )
				{
					$this->compare_meta_and_taxonomy_data( $post_id, $property['project_id'], $metadata_before, $taxonomy_terms_before );
				}
			}
		...
		do_action( "houzez_property_feed_post_import_properties_behomes", $this->import_id );
	...
	public function remove_old_properties()
	{
		...
		if ( !empty($this->properties) )
		{
			...
			foreach ($this->properties as $property)
			{
				$import_refs[] = $property['project_id'];
			}
