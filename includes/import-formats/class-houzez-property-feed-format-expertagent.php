		...
		foreach ( $this->properties as $property )
		{
			...
			if ( $inserted_updated !== false )
			{
				...
				else
				{
							...
							foreach ( $pictures->picture as $picture )
							{
								...
								if ( isset($picture->filename) && trim((string)$picture->filename) != '' )
								{
									...
									)
									{
										...
										{
											if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, (string)$property->property_reference, $url, $url, $description, 'image', $image_i, $modified ) === true )
											{
												$this->ping();
					...
					if ( $queued > 0 ) 
					{
						$this->log( $queued . ' photos added to download queue', (string)$property->property_reference, $post_id );
					}
				...
				do_action( "houzez_property_feed_property_imported_expertagent", $post_id, $property, $this->import_id, $this->instance_id );
