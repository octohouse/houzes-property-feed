<?php
/**
 * Class for managing the import process of a Pixxi JSON file
 *
 * @package WordPress
 */
if ( class_exists( 'Houzez_Property_Feed_Process' ) ) {

class Houzez_Property_Feed_Format_Pixxi extends Houzez_Property_Feed_Process {
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_pixxi", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_pixxi", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
	        ...
	        $display_address = $property['title'];
			...
			if ( $inserted_updated !== false )
			{
	            ...
	            if ( isset($property[strtolower($property['listingType']) . 'Param']['position']) && !empty($property[strtolower($property['listingType']) . 'Param']['position']) )
				{
					...
					{
						$lat = $explode_position[0];
						$lng = $explode_position[1];
					}
				}
				...
				{
					$this->log( 'Received status of ' . $status . ' that isn\'t mapped in the import settings', $property['id'], $post_id );
				...
				if ( isset($property['propertyType']) && !empty($property['propertyType']) && is_array($property['propertyType']) )
				{
					...
					{
						if ( 
							isset($taxonomy_mappings[$property_type]) && 
							!empty($taxonomy_mappings[$property_type]) 
						)
						{
							$term_ids[] = (int)$taxonomy_mappings[$property_type];
						}
						else
						{
							$this->log( 'Received property type of ' . $property_type . ' that isn\'t mapped in the import settings', $property['id'], $post_id );
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_pixxi', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
				...
				do_action( "houzez_property_feed_property_imported_pixxi", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_pixxi", $this->import_id );
