...
class Houzez_Property_Feed_Format_Inmovilla extends Houzez_Property_Feed_Process {
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_inmovilla", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_inmovilla", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', (string)$property->id, $post_id );

	                $my_post = array(
				    	'ID'          	 => $post_id,
				    	'post_title'     => wp_strip_all_tags( $display_address ),
				    	'post_excerpt'   => (string)$property->descrip1,
				    	'post_content' 	 => (string)$property->descrip1,
				    	'post_status'    => 'publish',
				  	);
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( strpos((string)$property->accion, 'Alquilar') !== false || strpos((string)$property->accion, 'Alquiler') !== false )
				{
                ...
                update_post_meta( $post_id, 'fave_property_bedrooms', ( ( isset($property->habitaciones) ) ? (string)$property->habitaciones : '' ) );
	            ...
	            update_post_meta( $post_id, 'fave_property_size', ( ( isset($property->m_cons) && !empty((string)$property->m_cons) ) ? (string)$property->m_cons : '' ) );
	            update_post_meta( $post_id, 'fave_property_size_prefix', ( ( isset($property->m_cons) && !empty((string)$property->m_cons) ) ? 'sqm' : '' ) );
	            update_post_meta( $post_id, 'fave_property_land', ( ( isset($property->m_parcela) && !empty((string)$property->m_parcela) ) ? (string)$property->m_parcela : '' ) );
	            update_post_meta( $post_id, 'fave_property_land_postfix', ( ( isset($property->m_parcela) && !empty((string)$property->m_parcela) ) ? 'sqm' : '' ) );
	            ...
	            if ( isset($property->ciudad) && (string)$property->ciudad != '' )
	            {
	                $address_parts[] = (string)$property->ciudad;
	            }
	            if ( isset($property->cp) && ( (string)$property->cp != '' ) )
	            {
	                $address_parts[] = (string)$property->cp;
	            }
	            ...
	            $lat = '';
	            $lng = '';
	            if ( isset($property->latitud) && !empty((string)$property->latitud) )
	            {
	                update_post_meta( $post_id, 'houzez_geolocation_lat', (string)$property->latitud );
	                $lat = (string)$property->latitud;
	            }
	            if ( isset($property->altitud) && !empty((string)$property->altitud) )
	            {
	                update_post_meta( $post_id, 'houzez_geolocation_long', (string)$property->altitud );
	                $lng = (string)$property->altitud;
	            }
	            ...
	            update_post_meta( $post_id, 'fave_property_zip', ( ( isset($property->cp) && (string)$property->cp != '' ) ? (string)$property->cp : '' ) );
				...
				if ( isset($property->accion) && (string)$property->accion != '' )
				{
					if ( isset($taxonomy_mappings[(string)$property->accion]) && !empty($taxonomy_mappings[(string)$property->accion]) )
					{
						wp_set_object_terms( $post_id, (int)$taxonomy_mappings[(string)$property->accion], "property_status" );
					}
					else
					{
						$this->log( 'Received status of ' . (string)$property->accion . ' that isn\'t mapped in the import settings', (string)$property->id, $post_id );

						$import_settings = $this->add_missing_mapping( $mappings, $mapping_name, (string)$property->accion, $this->import_id );
					}
				}
				...
				if ( isset($property->tipo_ofer) && !empty((string)$property->tipo_ofer) )
				{
					if ( isset($taxonomy_mappings[(string)$property->tipo_ofer]) && !empty($taxonomy_mappings[(string)$property->tipo_ofer]) )
					{
						wp_set_object_terms( $post_id, (int)$taxonomy_mappings[(string)$property->tipo_ofer], "property_type" );
					}
					else
					{
						$this->log( 'Received property type of ' . (string)$property->tipo_ofer . ' that isn\'t mapped in the import settings', (string)$property->id, $post_id );

						$import_settings = $this->add_missing_mapping( $mappings, "property_type", (string)$property->tipo_ofer, $this->import_id );
					}
				}
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_inmovilla', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
					...
					for ( $i = 0; $i <= 49; ++$i )
					{
						...
						{
							if ( 
								substr( strtolower((string)$property->{'foto' . $i}), 0, 2 ) == '//' || 
								substr( strtolower((string)$property->{'foto' . $i}), 0, 4 ) == 'http'
							)
							{
								$urls[] = array(
				...
				for ( $i = 0; $i <= 5; ++$i )
				{
					if ( isset($property->videos->{'video' . $i}) && trim((string)$property->videos->{'video' . $i}) != '' )
					{
						if ( 
							substr( strtolower((string)$property->videos->{'video' . $i}), 0, 2 ) == '//' || 
							substr( strtolower((string)$property->videos->{'video' . $i}), 0, 4 ) == 'http'
						)
						{
							// This is a URL
							$url = trim((string)$property->videos->{'video' . $i});
				...
				do_action( "houzez_property_feed_property_imported_inmovilla", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_inmovilla", $this->import_id );
