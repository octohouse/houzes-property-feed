<?php
/**
 * Class for managing the import process of a CASAFARI file
 *
 * @package WordPress
 */
if ( class_exists( 'Houzez_Property_Feed_Process' ) ) {

class Houzez_Property_Feed_Format_Casafari extends Houzez_Property_Feed_Process {
	...
	public function parse()
	{
		...
		while ( $more_properties )
		{
			...
			$body = apply_filters( 'houzez_property_feed_casafari_args', $body );
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_casafari", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_casafari", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
			...
			if ( $inserted_updated !== false )
			{
				...
				$poa = false;
				if ( isset($property['price_visible']) && $property['price_visible'] === false )
				{
					$poa = true;
				}
                ...
                if ( isset($property['living_area']) && !empty($property['living_area']) )
                {
                	update_post_meta( $post_id, 'fave_property_size', $property['living_area'] );
                	update_post_meta( $post_id, 'fave_property_size_prefix', 'Sq M' );
                }

                if ( isset($property['plot_area']) && !empty($property['plot_area']) )
                {
                	update_post_meta( $post_id, 'fave_property_land', $property['plot_area'] );
	            ...
	            if ( isset($property['location']['Locality']) && $property['location']['Locality'] != '' )
	            {
	                $address_parts[] = $property['location']['Locality'];
	            }
	            if ( isset($property['location']['City']) && $property['location']['City'] != '' )
	            {
	                $address_parts[] = $property['location']['City'];
	            }
	            if ( isset($property['location']['Region']) && $property['location']['Region'] != '' )
	            {
	                $address_parts[] = $property['location']['Region'];
	            }
	            if ( isset($property['location']['zipcode']) && $property['location']['zipcode'] != '' )
	            {
	                $address_parts[] = $property['location']['zipcode'];
	            }
	            ...
	            if ( isset($property['location']['coordinates']['visible']) && $property['location']['coordinates']['visible'] === true )
	            {
		            ...
		            if ( isset($property['location']['coordinates']['latitude']) && !empty($property['location']['coordinates']['latitude']) )
		            {
		                update_post_meta( $post_id, 'houzez_geolocation_lat', $property['location']['coordinates']['latitude'] );
		                $lat = $property['location']['coordinates']['latitude'];
		            }
		            if ( isset($property['location']['coordinates']['longitude']) && !empty($property['location']['coordinates']['longitude']) )
		            {
		                update_post_meta( $post_id, 'houzez_geolocation_long', $property['location']['coordinates']['longitude'] );
	            ...
	            update_post_meta( $post_id, 'fave_property_zip', ( ( isset($property['location']['zipcode']) ) ? $property['location']['zipcode'] : '' ) );
		            ...
		            switch ( $import_settings['agent_display_option'] )
		            {
		            	case "author_info":
		            	{
		            		foreach ( $import_settings['agent_display_option_rules'] as $rule )
		            		{
		            			...
		            			switch ( $rule['field'] )
		            			{
		            				case "Listing Agent ID":
		            				{
		            					$value_in_feed_to_check = isset($property['listing_agent'][0]['id']) ? $property['listing_agent'][0]['id'] : '';
		            					break;
		            				}
		            				case "Listing Agent Name":
		            				{
		            					$value_in_feed_to_check = isset($property['listing_agent'][0]['Name']) ? $property['listing_agent'][0]['Name'] : '';
		            					break;
		            				}
		            			}
	            ...
	            if ( isset($property['features_list']) && is_array($property['features_list']) )
				{
					foreach ( $property['features_list'] as $feature )
					{
				...
				if ( isset($property['businessTypeLocale']) && !empty($property['businessTypeLocale']) )
				{
					if ( isset($taxonomy_mappings[$property['businessTypeLocale']]) && !empty($taxonomy_mappings[$property['businessTypeLocale']]) )
					{
						wp_set_object_terms( $post_id, (int)$taxonomy_mappings[$property['businessTypeLocale']], "property_status" );
					}
					else
					{
						$this->log( 'Received status of ' . $property['businessTypeLocale'] . ' that isn\'t mapped in the import settings', $property['id'], $post_id );

						$import_settings = $this->add_missing_mapping( $mappings, $mapping_name, $property['businessTypeLocale'], $this->import_id );
					}
				}
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_casafari', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
				...
				{
					foreach ($property['files'] as $floorplan)
					{
						if ( 
							isset($floorplan['Filename']) && $floorplan['Filename'] != ''
							&&
							(
								substr( strtolower($floorplan['Filename']), 0, 2 ) == '//' || 
								substr( strtolower($floorplan['Filename']), 0, 4 ) == 'http'
							)
							&& 
							isset($floorplan['Category']) && strtolower($floorplan['Category']) == 'floorplan'
						)
						{
							...
							$floorplans[] = array( 
								...
								"fave_plan_image" => $floorplan['Filename']
							);
						}
					}
				}
				...
				do_action( "houzez_property_feed_property_imported_casafari", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_casafari", $this->import_id );
