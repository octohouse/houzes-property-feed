<?php
/**
 * Class for managing the import process of a PropConnect JSON file
 *
 * @package WordPress
 */
if ( class_exists( 'Houzez_Property_Feed_Process' ) ) {

class Houzez_Property_Feed_Format_Propconnect extends Houzez_Property_Feed_Process {
	...
	public function parse()
	{
		...
		$url = 'https://crm.propconnect.ae/api/v2/export/JSON/' . $import_settings['token'] . '/generic';
		...
		{
			$this->log_error( 'Failed to obtain JSON from ' . $url . ': ' . print_r($response, TRUE) );
			return false;
		}
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_propconnect", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_propconnect", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
			if ( !empty($start_at_property) )
			{
				...
				if ( $property['reference_number'] == $start_at_property )
				{
					// we found the property. We'll continue for this property onwards
					$this->log( 'Previous import failed to complete. Continuing from property ' . $property_row . ' with ID ' . $property['reference_number'] );
			...
			update_option( 'houzez_property_feed_property_' . $this->import_id, $property['reference_number'], false );
			
			$this->log( 'Importing property ' . $property_row . ' with reference ' . $property['reference_number'], $property['reference_number'], 0, '', false );
	        ...
	        $display_address = $property['title_en'];
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', $property['reference_number'], $post_id );

	                $my_post = array(
				    	'ID'          	 => $post_id,
				    	'post_title'     => wp_strip_all_tags( $display_address ),
				    	'post_excerpt'   => $property['description_en'],
					...
					{
						$this->log_error( 'Failed to update post. The error was as follows: ' . $post_id->get_error_message(), $property['reference_number'] );
					}
					else
					{
	        ...
	        {
	        	$this->log( 'This property hasn\'t been imported before. Inserting it', $property['reference_number'] );
				...
				if ( is_wp_error( $post_id ) ) 
				{
					$this->log_error( 'Failed to insert post. The error was as follows: ' . $post_id->get_error_message(), $property['reference_number'] );
				}
				else
				{
			...
			if ( $inserted_updated !== false )
			{
				...
				$this->log( 'Successfully ' . $inserted_updated . ' post', $property['reference_number'], $post_id );

				update_post_meta( $post_id, $imported_ref_key, $property['reference_number'] );

				update_post_meta( $post_id, '_property_import_data', json_encode($property, JSON_PRETTY_PRINT) );

				$department = isset($property['offering_type']) && strtolower($property['offering_type']) == 'rent' ? 'residential-lettings' : 'residential-sales';

				$poa = false;
				if ( isset($property['price_on_application']) && strtolower($property['price_on_application']) == 'yes' )
				{
					$poa = true;
				}
                ...
                update_post_meta( $post_id, 'fave_property_bedrooms', ( ( isset($property['bedroom']) && $property['bedroom'] != '' ) ? $property['bedroom'] : '' ) );
	            update_post_meta( $post_id, 'fave_property_bathrooms', ( ( isset($property['bathroom']) && $property['bathroom'] != '' ) ? $property['bathroom'] : '' ) );
	            update_post_meta( $post_id, 'fave_property_rooms', '' );
	            update_post_meta( $post_id, 'fave_property_garage', '' );
	            update_post_meta( $post_id, 'fave_property_id', $property['reference_number'] );

	            $size = ( ( isset($property['size']) && !empty($property['size']) ) ? round($property['size']) : '' );
	            update_post_meta( $post_id, 'fave_property_size', $size );
	            update_post_meta( $post_id, 'fave_property_size_prefix', ( !empty($size) ? 'sq ft' : '' ) );

	            $address_parts = array();
	            if ( isset($property['sub_community']) && $property['sub_community'] != '' )
	            {
	                $address_parts[] = $property['sub_community'];
	            }
	            if ( isset($property['community']) && $property['community'] != '' )
	            {
	                $address_parts[] = $property['community'];
	            }
	            if ( isset($property['city']) && $property['city'] != '' )
	            {
	                $address_parts[] = $property['city'];
	            }
	            if ( isset($property['emirate']) && $property['emirate'] != '' )
	            {
	                $address_parts[] = $property['emirate'];
	            }
		            	...
		            	case "author_info":
		            	{
		            		foreach ( $import_settings['agent_display_option_rules'] as $rule )
		            		{
		            			...
		            			switch ( $rule['field'] )
		            			{
		            				case "agent_id":
		            				{
		            					$value_in_feed_to_check = $property['agent']['id'];
		            					break;
		            				}
		            				case "agent_name":
		            				{
				...
				{
					wp_set_object_terms( $post_id, (int)$taxonomy_mappings[$status], "property_status" );
				}
				else
				{
					$this->log( 'Received status of ' . $status . ' that isn\'t mapped in the import settings', $property['reference_number'], $post_id );

					$import_settings = $this->add_missing_mapping( $mappings, $mapping_name, $status, $this->import_id );
				}
				...
				$term_ids = array();
				if ( isset($property['property_type']) && !empty($property['property_type']) && is_array($property['property_type']) )
				{
					foreach ( $property['propertyType'] as $property_type )
					{
						if ( 
							isset($taxonomy_mappings[$property['property_type']]) && 
							!empty($taxonomy_mappings[$property['property_type']]) 
						)
						{
							$term_ids[] = (int)$taxonomy_mappings[$property['property_type']];
						}
						else
						{
							$this->log( 'Received property type of ' . $property_type . ' that isn\'t mapped in the import settings', $property['reference_number'], $post_id );
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_propconnect', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
					...
					$this->log( 'Imported ' . count($urls) . ' photo URLs', $property['reference_number'], $post_id );
				}
				else
				{
					$media_ids = array();
					...
					if ( !empty($previous_import_media_ids) )
					{
						...
						if ( $explode_previous_import_media_ids[0] == $post_id )
						{
							...
							if ( isset($explode_previous_import_media_ids[1]) && !empty($explode_previous_import_media_ids[1]) )
							{
								...
								$this->log( 'Imported ' . count($media_ids) . ' images before failing in the previous import. Continuing from here', $property['reference_number'], $post_id );
							}
						}
					}
					...
					if ( isset($property['images']) && is_array($property['images']) && !empty($property['images']) )
					{
						foreach ( $property['images'] as $image )
						{
							...
							)
							{
								...
								{
									if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, $property['reference_number'], $url, $url, $description, 'image', $image_i, '' ) === true )
									{
										$this->ping();
									    ...
									    if ( is_wp_error( $tmp ) ) 
									    {
									        $this->log_error( 'An error occurred whilst importing ' . $url . '. The error was as follows: ' . $tmp->get_error_message(), $property['reference_number'], $post_id );
									    }
									    else
									    {
										    ...
										    if ( is_wp_error( $id ) ) 
										    {
										        ...
										        $this->log_error( 'ERROR: An error occurred whilst importing ' . $url . '. The error was as follows: ' . $id->get_error_message(), $property['reference_number'], $post_id );
										    }
										    else
										    {
										    	$media_ids[] = $id;
					...
					$this->log( 'Imported ' . count($media_ids) . ' photos (' . $new . ' new, ' . $existing . ' existing, ' . $deleted . ' deleted)', $property['reference_number'], $post_id );
					if ( $queued > 0 ) 
					{
						$this->log( $queued . ' photos added to download queue', $property['reference_number'], $post_id );
					}
				...
				do_action( "houzez_property_feed_property_imported_propconnect", $post_id, $property, $this->import_id, $this->instance_id );
				...
				if ( $inserted_updated == 'updated' )
				{
					$this->compare_meta_and_taxonomy_data( $post_id, $property['reference_number'], $metadata_before, $taxonomy_terms_before );
				}
			}
		...
		do_action( "houzez_property_feed_post_import_properties_propconnect", $this->import_id );
	...
	public function remove_old_properties()
	{
		...
		if ( !empty($this->properties) )
		{
			...
			foreach ($this->properties as $property)
			{
				$import_refs[] = $property['reference_number'];
			}
