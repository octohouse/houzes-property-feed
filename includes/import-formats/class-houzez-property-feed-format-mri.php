		...
		foreach ( $this->properties as $property )
		{
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( isset($property->property_summary->status) && !empty((string)$property->property_summary->status) )
				{
					...
					{
						wp_set_object_terms( $post_id, $taxonomy_mappings[(string)$property->property_summary->status], "property_status" );
					}
					else
					{
				...
				if ( isset($property->extra_info->prty_code) && isset($property->extra_info->prst_code) && !empty((string)$property->extra_info->prty_code) && !empty((string)$property->extra_info->prst_code) )
				{
					...
					{
						wp_set_object_terms( $post_id, $taxonomy_mappings[(string)$property->extra_info->prty_code . '-' . (string)$property->extra_info->prst_code], "property_type" );
					}
					else
					{
				...
				do_action( "houzez_property_feed_property_imported_mri", $post_id, $propert, $this->import_id );
