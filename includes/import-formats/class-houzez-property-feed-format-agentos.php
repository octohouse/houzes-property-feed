		...
		foreach ( $this->properties as $property )
		{
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', $property['OID'], $post_id );
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( isset($property['photos']) && !empty($property['photos']) )
				{
					foreach ( $property['photos'] as $image )
					{
						...
						if ( isset($image['PhotoType']) && strtolower($image['PhotoType']) == 'photo' )
						{
							...
							{
								if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, $property['OID'], $url, $url . $etag, $description, 'image', $image_i, '' ) === true )
								{
									$this->ping();
				...
				if ( $queued > 0 ) 
				{
					$this->log( $queued . ' photos added to download queue', $property['OID'], $post_id );
				}
				...
				do_action( "houzez_property_feed_property_imported_agentos", $post_id, $property, $this->import_id, $this->instance_id );
