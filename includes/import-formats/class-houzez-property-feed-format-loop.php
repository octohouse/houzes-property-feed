		...
		foreach ( $this->properties as $property )
		{
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', $property['listingId'], $post_id );
			...
			if ( $inserted_updated !== false )
			{
				...
				else
				{
					...
					if ( isset($property['images']) && is_array($property['images']) && !empty($property['images']) )
					{
						foreach ( $property['images'] as $image )
						{
							...
							)
							{
								...
								{
									if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, $property['listingId'], $url, $url, $description, 'image', $image_i, $modified ) === true )
									{
										$this->ping();
					...
					if ( $queued > 0 ) 
					{
						$this->log( $queued . ' photos added to download queue', $property['listingId'], $post_id );
					}
				...
				do_action( "houzez_property_feed_property_imported_loop", $post_id, $property, $this->import_id, $this->instance_id );
