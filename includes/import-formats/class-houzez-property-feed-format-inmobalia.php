<?php
/**
 * Class for managing the import process of an Inmobalia JSON file
 *
 * @package WordPress
 */
if ( class_exists( 'Houzez_Property_Feed_Process' ) ) {

class Houzez_Property_Feed_Format_Inmobalia extends Houzez_Property_Feed_Process {
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_inmobalia", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_inmobalia", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
			...
			if ( $inserted_updated !== false )
			{
            	...
            	{
            		if ( isset($property['salePriceShow']) && $property['salePriceShow'] !== true )
            		{
                		$poa = true;
                	}
            	}
            	...
            	{
            		if ( isset($property['rentalPriceShow']) && $property['rentalPriceShow'] !== true )
            		{
                		$poa = true;
                	}
            	}
	            ...
	            update_post_meta( $post_id, 'fave_property_country', $property['locationCountry']['codeIso'] );
	            ...
	            $featured = '0';
	            if ( isset($property['isFeatured']) && $property['isFeatured'] === true )
				{
			    	$featured = '1';
				}
		            	...
		            	case "author_info":
		            	{
		            		foreach ( $import_settings['agent_display_option_rules'] as $rule )
		            		{
		            			...
		            			switch ( $rule['field'] )
		            			{
		            				case "agency":
		            				{
		            					$value_in_feed_to_check = $property['agency'];
		            					break;
		            				}
		            			}
				...
				{
					if ( isset($taxonomy_mappings[$property['propertyType']['name']]) )
					{
						wp_set_object_terms( $post_id, (int)$taxonomy_mappings[$property['propertyType']['name']], "property_type" );
					}
					else
					{
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_inmobalia', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
				...
				do_action( "houzez_property_feed_property_imported_inmobalia", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_inmobalia", $this->import_id );
