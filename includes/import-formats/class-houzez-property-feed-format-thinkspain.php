...
class Houzez_Property_Feed_Format_Thinkspain extends Houzez_Property_Feed_Process {
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_thinkspain", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_thinkspain", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
			...
			update_option( 'houzez_property_feed_property_' . $this->import_id, (string)$property->unique_id, false );
			
			$this->log( 'Importing property ' . $property_row . ' with reference ' . (string)$property->unique_id, (string)$property->unique_id, 0, '', false );
			...
			$args = array(
	            ...
	            'meta_query' => array(
	            	array(
		            	...
		            	'value' => (string)$property->unique_id
		            )
	            )
	        );
			...
			if ( isset($property->town) && !empty((string)$property->town) )
			{
				$display_address[] = (string)$property->town;
			}
			if ( isset($property->province) && !empty((string)$property->province) )
			{
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $my_post = array(
				    	...
				    	'post_excerpt'   => (string)$property->description->es,
				    	'post_content' 	 => (string)$property->description->es,
				    	'post_status'    => 'publish',
				  	);
					...
					{
						$this->log_error( 'Failed to update post. The error was as follows: ' . $post_id->get_error_message(), (string)$property->unique_id );
					}
					else
					{
	        ...
	        {
	        	$this->log( 'This property hasn\'t been imported before. Inserting it', (string)$property->unique_id );
				...
				if ( is_wp_error( $post_id ) ) 
				{
					$this->log_error( 'Failed to insert post. The error was as follows: ' . $post_id->get_error_message(), (string)$property->unique_id );
				}
				else
				{
			...
			if ( $inserted_updated !== false )
			{
				...
				$this->log( 'Successfully ' . $inserted_updated . ' post', (string)$property->unique_id, $post_id );
	            ...
	            update_post_meta( $post_id, 'fave_property_size', ( ( isset($property->living_area) && !empty((string)$property->living_area) ) ? (string)$property->surface_area->built : '' ) );
	            update_post_meta( $post_id, 'fave_property_size_prefix', ( ( isset($property->living_area) && !empty((string)$property->living_area) ) ? 'Sq M' : '' ) );
	            update_post_meta( $post_id, 'fave_property_land', '' );
	            update_post_meta( $post_id, 'fave_property_land_postfix', '' );
	            ...
	            update_post_meta( $post_id, 'fave_property_country', 'Spain' );
				...
				if ( isset($property->property_type) )
				{
					...
					{
						$this->log( 'Received property type of ' . (string)$property->property_type . ' that isn\'t mapped in the import settings', (string)$property->unique_id, $post_id );
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_thinkspain', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
					...
					$this->log( 'Imported ' . count($urls) . ' photo URLs', (string)$property->unique_id, $post_id );
				}
				else
				{
					$media_ids = array();
					...
					if ( !empty($previous_import_media_ids) )
					{
						...
						if ( $explode_previous_import_media_ids[0] == $post_id )
						{
							...
							if ( isset($explode_previous_import_media_ids[1]) && !empty($explode_previous_import_media_ids[1]) )
							{
								...
								$this->log( 'Imported ' . count($media_ids) . ' images before failing in the previous import. Continuing from here', (string)$property->unique_id, $post_id );
							}
						}
					}
	                        ...
	                        if (!empty($images->photo))
	                        {
	                            foreach ($images->photo as $image)
	                            {
									...
									)
									{
										...
										{
											if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, (string)$property->unique_id, $url, $url, $description, 'image', $image_i, '' ) === true )
											{
												$this->ping();
											    ...
											    if ( is_wp_error( $tmp ) ) 
											    {
											        $this->log_error( 'An error occurred whilst importing ' . $url . '. The error was as follows: ' . $tmp->get_error_message(), (string)$property->unique_id, $post_id );
											    }
											    else
											    {
												    ...
												    if ( is_wp_error( $id ) ) 
												    {
												        ...
												        $this->log_error( 'ERROR: An error occurred whilst importing ' . $url . '. The error was as follows: ' . $id->get_error_message(), (string)$property->unique_id, $post_id );
												    }
												    else
												    {
												    	$media_ids[] = $id;
					...
					$this->log( 'Imported ' . count($media_ids) . ' photos (' . $new . ' new, ' . $existing . ' existing, ' . $deleted . ' deleted)', (string)$property->unique_id, $post_id );
					if ( $queued > 0 ) 
					{
						$this->log( $queued . ' photos added to download queue', (string)$property->unique_id, $post_id );
					}
				...
				do_action( "houzez_property_feed_property_imported_thinkspain", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_thinkspain", $this->import_id );
