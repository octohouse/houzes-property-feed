...
class Houzez_Property_Feed_Format_Xml extends Houzez_Property_Feed_Process {
	...
	public function import()
	{
		...
		foreach ( $this->properties as $property )
		{
			...
			$this->log( 'Importing property ' . $property_row . ' with reference ' . $property_id, $property_id, 0, '', false );
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', $property_id, $post_id );
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_xml', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
						...
						foreach ( $explode_media as $media_item )
						{
			                ...
			                if ( $matches !== FALSE && isset($matches[0]) && is_array($matches[0]) && !empty($matches[0]) )
			                {
			                    foreach ( $matches[0] as $match )
			                    {
			                        ...
			                        $urls_found = $property->xpath('/' . $property_node . $field_name);

			                        $value_to_check = '';
									if ( $urls_found === FALSE || empty($urls_found) )
						            {
						                //continue;
						            }
						            else
					...
					if ( $image_mapping_specified )
					{
						...
						$this->log( 'Imported ' . count($urls) . ' photo URLs', $property_id, $post_id );
					}
					else
					{
				...
				do_action( "houzez_property_feed_property_imported_xml", $post_id, $property, $this->import_id, $this->instance_id );
