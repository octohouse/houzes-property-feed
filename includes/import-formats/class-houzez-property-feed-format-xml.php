...
class Houzez_Property_Feed_Format_Xml extends Houzez_Property_Feed_Process {
	...
	public function parse()
	{
		...
		{
			$this->log_error( 'Please ensure you have a field specified that we can use as the property record identifer in the import setting under the \'Format\' tab' );
			return false;
		}

		if ( !isset($import_settings['property_id_node']) || ( isset($import_settings['property_id_node']) && empty($import_settings['property_id_node']) ) )
		{
			$this->log_error( 'Please ensure you have a field specified that we can use as the unique property identifer in the import setting under the \'Format\' tab and that is has a value set in the XML' );
			return false;
		}
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( isset($import_settings['image_fields']) && !empty($import_settings['image_fields']) )
				{
					...
					foreach ( $explode_media as $media_item )
					{
						...
						$url_xpath = $explode_media_item[0];
						$description_xpath = isset($explode_media_item[1]) ? $explode_media_item[1] : '';

						$description = '';
						if ( !empty($description_xpath) )
						{
							if ( substr($description_xpath, 0, 1) != '/' )
							{
								// Fixed string
							...
							{
								$descriptions = $property->xpath('/' . $property_node . $description_xpath);
						...
						$urls = $property->xpath('/' . $property_node . $url_xpath);
