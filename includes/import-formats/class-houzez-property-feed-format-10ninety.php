	...
	public function parse( $test = false )
	{
		...
		if ($xml !== FALSE)
		{
			foreach ($xml->property as $property)
			{
                ...
                {
                	$this->write_to_queue( (string)$property->AGENT_REF, $property );
                }
		...
		foreach ( $this->properties as $property )
		{
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', (string)$property->AGENT_REF, $post_id );
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_10ninety', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
					...
					$this->log( 'Imported ' . count($urls) . ' photo URLs', (string)$property->AGENT_REF, $post_id );
				}
				else
				{
					$media_ids = array();
				...
				do_action( "houzez_property_feed_property_imported_10ninety", $post_id, $property, $this->import_id, $this->instance_id );
