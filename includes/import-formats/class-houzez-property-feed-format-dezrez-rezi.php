		...
		foreach ( $this->properties as $property )
		{
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', $property['RoleId'], $post_id );
			...
			if ( $inserted_updated !== false )
			{
				...
				else
				{
					...
					if ( isset($property['Images']) && !empty($property['Images']) )
					{
						foreach ( $property['Images'] as $image )
						{
							...
							)
							{
								...
								{
									if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, $property['RoleId'], $url, $url, $description, 'image', $image_i, '' ) === true )
									{
										$this->ping();
					...
					if ( $queued > 0 ) 
					{
						$this->log( $queued . ' photos added to download queue', $property['RoleId'], $post_id );
					}
				...
				do_action( "houzez_property_feed_property_imported_dezrez_rezi", $post_id, $property, $this->import_id, $this->instance_id );
