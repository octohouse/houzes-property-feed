<?php
/**
 * Class for managing the import process of a Amplify Syndication API file
 *
 * @package WordPress
 */
if ( class_exists( 'Houzez_Property_Feed_Process' ) ) {

class Houzez_Property_Feed_Format_Amplify_Syndication extends Houzez_Property_Feed_Process {
	...
	public function parse()
	{
		...
		/*if ( ( isset($import_settings['only_updated']) && $import_settings['only_updated'] == 'yes' ) || !isset($import_settings['only_updated']) )
        {
        	...
        	{
        		$additional_url = ' and date(ModificationTimestamp) ge ' . date("Y-m-d");
		...
		foreach ( $statuses as $status )
		{
			...
			$url = 'https://query.ampre.ca/odata/Property?$top=1000&$filter=StandardStatus eq %27' . urlencode($status) . '%27' . $additional_url;
			...
			while ( $more_properties )
			{
				...
				if ( is_array( $response ) )
				{
					...
					if ( $json !== FALSE && is_array($json) )
					{
						...
						{
							$this->log_error( 'Error received from Amplify API: ' . print_r($json['error'], true) );
							return false;
						}
						...
						else
						{
							...
							sleep(1); // Stay within rate limiting
							++$current_page;
						}
					}
					else
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_amplify_syndication", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_amplify_syndication", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_amplify_syndication', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
				...
				do_action( "houzez_property_feed_property_imported_amplify_syndication", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_amplify_syndication", $this->import_id );
