<?php
/**
 * Class for managing the import process of an Idealista JSON file
 *
 * @package WordPress
 */
...
class Houzez_Property_Feed_Format_Idealista extends Houzez_Property_Feed_Process {
	...
	public function parse()
	{
		...
		if ($json !== FALSE)
		{
			$this->log("Parsing properties on page");


			if ( isset($json['customerProperties']) )
			{
				foreach ($json['customerProperties'] as $property)
				{
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_idealista", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_idealista", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
			if ( !empty($start_at_property) )
			{
				...
				if ( $property['propertyCode'] == $start_at_property )
				{
					// we found the property. We'll continue for this property onwards
					$this->log( 'Previous import failed to complete. Continuing from property ' . $property_row . ' with ID ' . $property['propertyCode'] );
			...
			update_option( 'houzez_property_feed_property_' . $this->import_id, $property['propertyCode'], false );
			
			$this->log( 'Importing property ' . $property_row . ' with reference ' . $property['propertyCode'], $property['propertyCode'], 0, '', false );
	        ...
	        if ( isset($property['propertyAddress']['addressStreetName']) && !empty($property['propertyAddress']['addressStreetName']) )
	        {
	        	$display_address[] = $property['propertyAddress']['addressStreetName'];
	        }
	        if ( isset($property['propertyAddress']['addressTown']) && !empty($property['propertyAddress']['addressTown']) )
	        {
	        	$display_address[] = $property['propertyAddress']['addressTown'];
	        }
	        $display_address = implode(", ", $display_address);

			$post_content = '';
			if ( isset($property['propertyDescriptions']) && !empty($property['propertyDescriptions']) )
			{
				foreach ( $property['propertyDescriptions'] as $description )
				{
					if ( isset($description['descriptionLanguage']) && strtolower($description['descriptionLanguage']) == 'english' )
					{
						$post_content = $description['descriptionText'];
					}
				}
			}
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', $property['propertyCode'], $post_id );
					...
					{
						$this->log_error( 'Failed to update post. The error was as follows: ' . $post_id->get_error_message(), $property['propertyCode'] );
					}
					else
					{
	        ...
	        {
	        	$this->log( 'This property hasn\'t been imported before. Inserting it', $property['propertyCode'] );
				...
				if ( is_wp_error( $post_id ) ) 
				{
					$this->log_error( 'Failed to insert post. The error was as follows: ' . $post_id->get_error_message(), $property['propertyCode'] );
				}
				else
				{
			...
			if ( $inserted_updated !== false )
			{
				...
				$this->log( 'Successfully ' . $inserted_updated . ' post', $property['propertyCode'], $post_id );

				update_post_meta( $post_id, $imported_ref_key, $property['propertyCode'] );

				update_post_meta( $post_id, '_property_import_data', json_encode($property, JSON_PRETTY_PRINT) );

				$department = isset($property['propertyOperation']) && $property['propertyOperation'] == 'rent' ? 'residential-lettings' : 'residential-sales';
                ...
                {
            		$price = '';
            		if ( isset($property['propertyOperation']['operationPrice']) && !empty($property['propertyOperation']['operationPrice']) )
            		{
                		$price = round(preg_replace("/[^0-9.]/", '', $property['propertyOperation']['operationPrice']));
                	}
                ...
                if ( isset($property['propertyFeatures']['featuresAreaConstructed']) && !empty($property['propertyFeatures']['featuresAreaConstructed']) )
                {
                	update_post_meta( $post_id, 'fave_property_size', $property['propertyFeatures']['featuresAreaConstructed'] );
                	update_post_meta( $post_id, 'fave_property_size_prefix', 'Sq M' );
                }

                if ( isset($property['propertyFeatures']['featuresAreaPlot']) && !empty($property['propertyFeatures']['featuresAreaPlot']) )
                {
                	update_post_meta( $post_id, 'fave_property_land', $property['propertyFeatures']['featuresAreaPlot'] );
                	update_post_meta( $post_id, 'fave_property_land_postfix', 'Sq M' );
                }

                update_post_meta( $post_id, 'fave_property_bedrooms', ( ( isset($property['propertyFeatures']['featuresBedroomNumber']) ) ? $property['propertyFeatures']['featuresBedroomNumber'] : '' ) );
	            update_post_meta( $post_id, 'fave_property_bathrooms', ( ( isset($property['propertyFeatures']['featuresBathroomNumber']) ) ? $property['propertyFeatures']['featuresBathroomNumber'] : '' ) );
	            update_post_meta( $post_id, 'fave_property_rooms', '' );

	            update_post_meta( $post_id, 'fave_property_garage', '' );
	            update_post_meta( $post_id, 'fave_property_id', $property['propertyReference'] );

	            $address_parts = array();
	            if ( isset($property['propertyAddress']['addressStreetName']) && $property['propertyAddress']['addressStreetName'] != '' )
	            {
	                $address_parts[] = $property['propertyAddress']['addressStreetName'];
	            }
	            if ( isset($property['propertyAddress']['addressTown']) && $property['propertyAddress']['taddressTownown'] != '' )
	            {
	                $address_parts[] = $property['propertyAddress']['addressTown'];
	            }
	            ...
	            $lat = '';
	            $lng = '';
	            if ( isset($property['propertyAddress']['addressCoordinatesLatitude']) && !empty($property['propertyAddress']['addressCoordinatesLatitude']) )
	            {
	                update_post_meta( $post_id, 'houzez_geolocation_lat', $property['propertyAddress']['addressCoordinatesLatitude'] );
	                $lat = $property['propertyAddress']['addressCoordinatesLatitude'];
	            }
	            if ( isset($property['propertyAddress']['addressCoordinatesLongitude']) && !empty($property['propertyAddress']['addressCoordinatesLongitude']) )
	            {
	                update_post_meta( $post_id, 'houzez_geolocation_long', $property['propertyAddress']['addressCoordinatesLongitude'] );
	                $lng = $property['propertyAddress']['addressCoordinatesLongitude'];
	            }
	            ...
	            update_post_meta( $post_id, 'fave_property_zip', ( ( isset($property['propertyAddress']['addressPostalCode']) ) ? $property['propertyAddress']['addressPostalCode'] : '' ) );
		            	...
		            	case "author_info":
		            	{
		            		foreach ( $import_settings['agent_display_option_rules'] as $rule )
		            		{
		            			...
		            			switch ( $rule['field'] )
		            			{
		            				case "contactName":
		            				{
		            					$value_in_feed_to_check = isset($property['propertyContact']['contactName']) ? $property['propertyContact']['contactName'] : '';
		            					break;
		            				}
		            			}
				...
				if ( isset($property['propertyFeatures']['featuresType']) )
				{
					if ( isset($taxonomy_mappings[$property['propertyFeatures']['featuresType']]) && !empty($taxonomy_mappings[$property['propertyFeatures']['featuresType']]) )
					{
						wp_set_object_terms( $post_id, (int)$taxonomy_mappings[$property['propertyFeatures']['featuresType']], "property_type" );
					}
					else
					{
						$this->log( 'Received property type of ' . $property['propertyFeatures']['featuresType'] . ' that isn\'t mapped in the import settings', $property['propertyCode'], $post_id );

						$import_settings = $this->add_missing_mapping( $mappings, 'property_type', $property['propertyFeatures']['featuresType'], $this->import_id );
					}
				}
				...
				foreach ( $location_taxonomies as $location_taxonomy )
				{
					...
					if ( !empty($address_field_to_use) )
					{
						...
						if ( isset($property['propertyAddress'][$address_field_to_use]) && !empty($property['propertyAddress'][$address_field_to_use]) )
		            	{
		            		$term = term_exists( trim($property['propertyAddress'][$address_field_to_use]), $location_taxonomy);
							...
							else
							{
								...
								{
									$term = wp_insert_term( trim($property['propertyAddress'][$address_field_to_use]), $location_taxonomy );
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_idealista', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
					...
					$this->log( 'Imported ' . count($urls) . ' photo URLs', $property['propertyCode'], $post_id );
				}
				else
				{
					$media_ids = array();
					...
					if ( !empty($previous_import_media_ids) )
					{
						...
						if ( $explode_previous_import_media_ids[0] == $post_id )
						{
							...
							if ( isset($explode_previous_import_media_ids[1]) && !empty($explode_previous_import_media_ids[1]) )
							{
								...
								$this->log( 'Imported ' . count($media_ids) . ' images before failing in the previous import. Continuing from here', $property['propertyCode'], $post_id );
							}
						}
					}
					...
					if (isset($property['propertyImages']) && !empty($property['propertyImages']))
					{
						foreach ($property['propertyImages'] as $image)
						{
							...
							)
							{
								...
								// This is a URL
								$description = ( (isset($image['imageLabel'])) ? $image['imageLabel'] : '' );
								...
								{
									if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, $property['propertyCode'], $url, $url, $description, 'image', $image_i, '' ) === true )
									{
										$this->ping();
									    ...
									    if ( is_wp_error( $tmp ) ) 
									    {
									        $this->log_error( 'An error occurred whilst importing ' . $url . '. The error was as follows: ' . $tmp->get_error_message(), $property['propertyCode'], $post_id );
									    }
									    else
									    {
										    ...
										    if ( is_wp_error( $id ) ) 
										    {
										        ...
										        $this->log_error( 'ERROR: An error occurred whilst importing ' . $url . '. The error was as follows: ' . $id->get_error_message(), $property['propertyCode'], $post_id );
										    }
										    else
										    {
										    	$media_ids[] = $id;
					...
					$this->log( 'Imported ' . count($media_ids) . ' photos (' . $new . ' new, ' . $existing . ' existing, ' . $deleted . ' deleted)', $property['propertyCode'], $post_id );
					if ( $queued > 0 ) 
					{
						$this->log( $queued . ' photos added to download queue', $property['propertyCode'], $post_id );
					}
				...
				do_action( "houzez_property_feed_property_imported_idealista", $post_id, $property, $this->import_id, $this->instance_id );
				...
				if ( $inserted_updated == 'updated' )
				{
					$this->compare_meta_and_taxonomy_data( $post_id, $property['propertyCode'], $metadata_before, $taxonomy_terms_before );
				}
			}
		...
		do_action( "houzez_property_feed_post_import_properties_idealista", $this->import_id );
	...
	public function remove_old_properties()
	{
		...
		if ( !empty($this->properties) )
		{
			...
			foreach ($this->properties as $property)
			{
				$import_refs[] = $property['propertyCode'];
			}
