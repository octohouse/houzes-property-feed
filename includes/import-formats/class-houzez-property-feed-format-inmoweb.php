...
class Houzez_Property_Feed_Format_Inmoweb extends Houzez_Property_Feed_Process {
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_inmoweb", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_inmoweb", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', (string)$property_attributes['id'], $post_id );
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( isset($property->operacion) && (string)$property->operacion == 'Alquiler' )
				{
                ...
                update_post_meta( $post_id, 'fave_property_bedrooms', ( ( isset($property->dormitorios) ) ? (string)$property->dormitorios : '' ) );
	            ...
	            update_post_meta( $post_id, 'fave_property_size', ( ( isset($property->superficies->construida) && !empty((string)$property->superficies->construida) ) ? (string)$property->superficies->construida : '' ) );
	            update_post_meta( $post_id, 'fave_property_size_prefix', ( ( isset($property->superficies->construida) && !empty((string)$property->superficies->construida) ) ? 'Sq M' : '' ) );
	            update_post_meta( $post_id, 'fave_property_land', ( ( isset($property->superficies->parcela) && !empty((string)$property->superficies->parcela) ) ? (string)$property->superficies->parcela : '' ) );
	            update_post_meta( $post_id, 'fave_property_land_postfix', ( ( isset($property->superficies->parcela) && !empty((string)$property->superficies->parcela) ) ? 'Sq M' : '' ) );
	            ...
	            if ( isset($property->localizacion->poblacion) && (string)$property->localizacion->poblacion != '' )
	            {
	                $address_parts[] = (string)$property->localizacion->poblacion;
	            }
	            if ( isset($property->localizacion->provincia) && (string)$property->localizacion->provincia != '' )
	            {
	                $address_parts[] = (string)$property->localizacion->provincia;
	            }
	            ...
	            $lat = '';
	            $lng = '';
	            if ( isset($property->localizacion->latitud) && !empty((string)$property->localizacion->latitud) )
	            {
	                update_post_meta( $post_id, 'houzez_geolocation_lat', (string)$property->localizacion->latitud );
	                $lat = (string)$property->localizacion->latitud;
	            }
	            if ( isset($property->localizacion->longitud) && !empty((string)$property->localizacion->longitud) )
	            {
	                update_post_meta( $post_id, 'houzez_geolocation_long', (string)$property->localizacion->longitud );
	                $lng = (string)$property->localizacion->longitud;
	            }
	            update_post_meta( $post_id, 'fave_property_location', $lat . "," . $lng . ",14" );
	            update_post_meta( $post_id, 'fave_property_country',(string)$property->localizacion->pais );
	            
	            update_post_meta( $post_id, 'fave_property_address', '' );
	            update_post_meta( $post_id, 'fave_property_zip', ( ( isset($property->localizacion->cp) ) ? (string)$property->localizacion->cp : '' ) );
				...
				if ( isset($property->status_id) && !empty((string)$property->status_id) )
				{
					...
					{
						$this->log( 'Received status of ' . (string)$property->status_id . ' that isn\'t mapped in the import settings', (string)$property_attributes['id'], $post_id );
				...
				{
					if ( isset($taxonomy_mappings[(string)$property->familia]) && !empty($taxonomy_mappings[(string)$property->familia]) )
					{
						wp_set_object_terms( $post_id, (int)$taxonomy_mappings[(string)$property->familia], "property_type" );
					}
					else
					{
						$this->log( 'Received property type of ' . (string)$property->familia . ' that isn\'t mapped in the import settings', (string)$property_attributes['id'], $post_id );

						$import_settings = $this->add_missing_mapping( $mappings, 'property_type', (string)$property->familia, $this->import_id );
					}
				}
				...
				foreach ( $location_taxonomies as $location_taxonomy )
				{
					...
					if ( !empty($address_field_to_use) )
					{
						...
						if ( isset($property->localizacion->{$address_field_to_use}) && !empty((string)$property->localizacion->{$address_field_to_use}) )
		            	{
		            		$term = term_exists( trim((string)$property->localizacion->{$address_field_to_use}), $location_taxonomy);
							if ( $term !== 0 && $term !== null && isset($term['term_id']) )
							{
							...
							else
							{
								...
								{
									$term = wp_insert_term( trim((string)$property->localizacion->{$address_field_to_use}), $location_taxonomy );
									if ( is_array($term) && isset($term['term_id']) )
									{
										$location_term_ids[] = (int)$term['term_id'];
									}
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_inmoweb', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
					...
					if (isset($property->imagenes) && !empty($property->imagenes))
	                {
                        foreach ($property->imagenes as $imagenes)
                        {
                        	if (isset($imagenes->imagen))
			                {
		                        ...
		                        {
		                        	//$this->log( print_r($image, true), (string)$property_attributes['id'], $post_id );
					...
					$this->log( 'Imported ' . count($urls) . ' photo URLs', (string)$property_attributes['id'], $post_id );
				}
				else
				{
					$media_ids = array();
                        	...
                        	if (isset($imagenes->imagen))
			                {
		                        foreach ($imagenes->imagen as $image)
		                        {
									...
									)
									{
										...
										{
											if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, (string)$property_attributes['id'], $url, $url, $description, 'image', $image_i, $modified ) === true )
											{
												$this->ping();
					...
					if ( $queued > 0 ) 
					{
						$this->log( $queued . ' photos added to download queue', (string)$property_attributes['id'], $post_id );
					}
				...
				do_action( "houzez_property_feed_property_imported_inmoweb", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_inmoweb", $this->import_id );
