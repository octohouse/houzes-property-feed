<?php
/**
 * Class for managing the import process of an XML2U XML file
 *
 * @package WordPress
 */
if ( class_exists( 'Houzez_Property_Feed_Process' ) ) {

class Houzez_Property_Feed_Format_Xml2u extends Houzez_Property_Feed_Process {
	...
	public function parse()
	{
		...
		{
			$this->log_error( "Failed to obtain XML. Received header code " . $response_code . ". Dump of response as follows: " . print_r($response, TRUE) );
		...
		if ( $xml !== FALSE )
		{
			if ( isset($xml->Clients->Client->properties->Property) )
			{
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_xml2u", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_xml2u", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
	        ...
	        $display_address = (string)$property->Description->title;
			if ( trim($display_address) == '' )
			{
				$display_address = (string)$property->Address->location;
			}
			if ( trim($display_address) == '' )
			{
				$display_address = (string)$property->Address->subRegion;
			}
	        ...
	        if ($property_query->have_posts())
	        {
	            ...
	            while ($property_query->have_posts())
	            {
	                ...
	                $this->log( 'This property has been imported before. Updating it', (string)$property->propertyid, $post_id );

	                $my_post = array(
				    	'ID'          	 => $post_id,
				    	'post_title'     => wp_strip_all_tags( $display_address ),
				    	'post_excerpt'   => (string)$property->Description->shortDescription,
				    	'post_content' 	 => (isset($property->Description->description->en) ? (string)$property->Description->description->en : ''),
				    	'post_status'    => 'publish',
				  	);
			...
			if ( $inserted_updated !== false )
			{
                ...
                {
                	$price = '';
					if ( isset($property->Price->price) && is_numeric((string)$property->Price->price) )
					{
						$price = round(preg_replace("/[^0-9.]/", '', (string)$property->Price->price));
					}

                    update_post_meta( $post_id, 'fave_property_price_prefix', ( ( isset($property->Price->prefix) ) ? (string)$property->Price->prefix : '' ) );
                ...
                update_post_meta( $post_id, 'fave_property_bedrooms', ( ( isset($property->Description->beds) ) ? (string)$property->Description->beds : '' ) );
	            update_post_meta( $post_id, 'fave_property_bathrooms', ( ( isset($property->Description->fullBathrooms) ) ? (string)$property->Description->fullBathrooms : '' ) );
	            update_post_meta( $post_id, 'fave_property_rooms', ( ( isset($property->Description->receptionRooms) ) ? (string)$property->Description->receptionRooms : '' ) );
	            
	            update_post_meta( $post_id, 'fave_property_size', ( ( isset($property->Description->FloorSize->floorSize) && !empty((string)$property->Description->FloorSize->floorSize) ) ? (string)$property->Description->FloorSize->floorSize : '' ) );
	            update_post_meta( $post_id, 'fave_property_size_prefix', ( ( isset($property->Description->FloorSize->floorSizeUnits) ) ? (string)$property->Description->FloorSize->floorSizeUnits : '' ) );
	            update_post_meta( $post_id, 'fave_property_land', ( ( isset($property->Description->PlotSize->plotSize) && !empty((string)$property->Description->PlotSize->plotSize) ) ? (string)$property->Description->PlotSize->plotSize : '' ) );
	            update_post_meta( $post_id, 'fave_property_land_postfix', ( ( isset($property->Description->PlotSize->plotSizeUnits) ) ? (string)$property->Description->PlotSize->plotSizeUnits : '' ) );
				
	            update_post_meta( $post_id, 'fave_property_garage', '' );
	            update_post_meta( $post_id, 'fave_property_id', (string)$property->ref );

	            $address_parts = array();
	            if ( isset($property->Address->location) && (string)$property->Address->location != '' )
	            {
	                $address_parts[] = (string)$property->Address->location;
	            }
	            if ( isset($property->Address->subRegion) && (string)$property->Address->subRegion != '' )
	            {
	                $address_parts[] = (string)$property->Address->subRegion;
	            }
	            ...
	            update_post_meta( $post_id, 'fave_property_country',(string)$property->Address->country );
		            	...
		            	case "author_info":
		            	{
		            		foreach ( $import_settings['agent_display_option_rules'] as $rule )
		            		{
		            			...
		            			switch ( $rule['field'] )
		            			{
		            				case "companyName":
		            				{
		            					$value_in_feed_to_check = isset($property->ListingContact->companyName) ? (string)$property->ListingContact->companyName : '';
		            				}
		            			}
	            ...
	            for ( $i = 1; $i <= 10; ++$i )
				{
					if ( isset($property->Features->{'Feature' . $i}) && trim((string)$property->Features->{'Feature' . $i}) != '' )
					{
						$feature = (string)$property->Features->{'Feature' . $i};
				...
				if ( isset($property->Description->propertyType) )
				{
					if ( isset($taxonomy_mappings[(string)$property->Description->propertyType]) && !empty($taxonomy_mappings[(string)$property->Description->propertyType]) )
					{
						wp_set_object_terms( $post_id, (int)$taxonomy_mappings[(string)$property->Description->propertyType], "property_type" );
					}
					else
					{
						$this->log( 'Received property type of ' . (string)$property->Description->propertyType . ' that isn\'t mapped in the import settings', (string)$property->propertyid, $post_id );

						$import_settings = $this->add_missing_mapping( $mappings, 'property_type', (string)$property->Description->propertyType, $this->import_id );
					}
				}
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_xml2u', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
				...
				else
				{
	                        ...
	                        if (!empty($images->image))
	                        {
	                            foreach ($images->image as $image)
	                            {
									...
									)
									{
										...
										{
											if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, (string)$property->propertyid, $url, $url, $description, 'image', $image_i, '' ) === true )
											{
												$this->ping();
					...
					if ( $queued > 0 ) 
					{
						$this->log( $queued . ' photos added to download queue', (string)$property->propertyid, $post_id );
					}
				...
				do_action( "houzez_property_feed_property_imported_xml2u", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_xml2u", $this->import_id );
