...
class Houzez_Property_Feed_Format_Domus extends Houzez_Property_Feed_Process {
	...
	public function parse()
	{
		...
		$statuses_to_exclude = apply_filters( 'houzez_property_feed_domus_exclude_statuses', array('Let', 'Sold') );
		...
		$response = wp_remote_get( $import_settings['xml_url'] . '/search?items=9999&includeUnavailable=true', array( 'timeout' => 120 ) );
		...
		{
			$this->log_error( "Failed to obtain sales properties XML. Dump of response as follows: " . print_r($response, TRUE) );
        ...
        {
        	// Failed to parse XML
        	$this->log_error( 'Failed to parse sales properties XML file: ' . $contents );
		...
		$response = wp_remote_get( $import_settings['xml_url'] . '/search?sales=false&items=9999&includeUnavailable=true', array( 'timeout' => 120 ) );
		if ( !is_wp_error($response) && is_array( $response ) ) 
		{
			$contents = $response['body'];
		}
		...
		{
			$this->log_error( "Failed to obtain lettings properties XML. Dump of response as follows: " . print_r($response, TRUE) );
        ...
        {
        	// Failed to parse XML
        	$this->log_error( 'Failed to parse lettings properties XML file: ' . $contents );
		...
		$response = wp_remote_get( $import_settings['xml_url'] . '/featured', array( 'timeout' => 120 ) );
		if ( !is_wp_error($response) && is_array( $response ) ) 
		{
			$contents = $response['body'];
	        ...
	        {
	        	// Failed to parse XML
	        	$this->log_error( 'Failed to parse features properties XML file: ' . $contents );
	        }
        }
		else
		{
			$this->log_error( "Failed to obtain featured properties XML. Dump of response as follows: " . print_r($response, TRUE) );
		}
	...
	private function get_property( $id )
	{
		...
		$response = wp_remote_get( $import_settings['xml_url'] . '/property?propertyID=' . $id, array( 'timeout' => 120 ) );
		if ( !is_wp_error($response) && is_array( $response ) ) 
		{
			$contents = $response['body'];
		}
		...
		{
			// Failed to parse XML
        	$this->log_error( 'Failed to parse property XML file from ' . $import_settings['xml_url'] . '/property?propertyID=' . $id . '. Possibly invalid XML: ' . $contents );
	...
	public function import()
	{
        ...
        do_action( "houzez_property_feed_pre_import_properties_domus", $this->properties, $this->import_id );

        $this->properties = apply_filters( "houzez_property_feed_properties_due_import", $this->properties, $this->import_id );
        $this->properties = apply_filters( "houzez_property_feed_properties_due_import_domus", $this->properties, $this->import_id );
		...
		foreach ( $this->properties as $property )
		{
	        ...
	        $display_address = (string)$property->address->advertising;

	        $post_content = '';
	        if ( isset($property->description) && (string)$property->description != '' )
	        {
	        	$post_content .= '<p>' . (string)$property->description . '</p>';
	        }
	        if ( isset($property->location) && (string)$property->location != '' )
	        {
	        	$post_content .= '<p>' . (string)$property->location . '</p>';
	        }
			                    ...
			                    foreach ($floor->rooms as $rooms)
			                    {
			                        if (!empty($rooms->room))
			                        {
			                            foreach ($rooms->room as $room)
			                            {
											...
											$room_content .= ( isset($room->size) && !empty((string)$room->size) ) ? ' (' . (string)$room->size . ')' : '';
	        ...
	        {
	        	$post_content .= '<p>' . (string)$property->additional . '</p>';
	        }
			...
			if ( $inserted_updated !== false )
			{
                ...
                else
                {
                    ...
                    update_post_meta( $post_id, 'fave_property_price_prefix', ( ( isset($property->pricequalifier) ) ? (string)$property->pricequalifier : '' ) );
                    ...
                    if ( $department == 'residential-lettings' && isset($property->rentFrequency) )
                    {
						...
						{
							case "per month": { $rent_frequency = 'pcm'; break; }
							case "per week": { $rent_frequency = 'pw'; break; }
						}
	            ...
	            update_post_meta( $post_id, 'fave_property_id', ( ( isset($property->reference) ) ? (string)$property->reference : '' ) );
	            ...
	            update_post_meta( $post_id, 'fave_featured', ( in_array((string)$property->id, $this->featured_properties) ? 'yes' : '' ) );
				...
				if ( 
					...
					apply_filters('houzez_property_feed_images_stored_as_urls_domus', false, $post_id, $property, $this->import_id) === true
				)
				{
					$urls = array();
	                        ...
	                        if (!empty($photos->photo))
	                        {
	                            foreach ($photos->photo as $photo)
	                            {
									...
									)
									{
										...
										{
											if ( apply_filters( 'houzez_property_feed_import_media', true, $this->import_id, $post_id, (string)$property->id, $url, $url, $description, 'image', $image_i, $modified ) === true )
											{
												$this->ping();
                ...
                {
                	foreach ($property->floorplans as $floorplans_xml)
                    {
                        if (!empty($floorplans_xml->floorplan))
                        {
                            foreach ($floorplans_xml->floorplan as $floorplan)
                            {
								if ( 
								...
								{
									$floorplans[] = array( 
										"fave_plan_title" => ( (isset($floorplan->caption)) ? (string)$floorplan->caption : __( 'Floorplan', 'houzezpropertyfeed' ) ), 
										"fave_plan_image" => (string)$floorplan->url
									);
								}
							}
						}
				...
				$this->log( 'Imported ' . count($media_ids) . ' EPCs (' . $new . ' new, ' . $existing . ' existing, ' . $deleted . ' deleted)', (string)$property->id, $post_id );
				
				update_post_meta( $post_id, 'fave_video_url', '' );
				update_post_meta( $post_id, 'fave_virtual_tour', '' );

				if (isset($property->propertyTourURL) && !empty((string)$property->propertyTourURL))
                {
					// This is a URL
				...
				do_action( "houzez_property_feed_property_imported_domus", $post_id, $property, $this->import_id, $this->instance_id );
		...
		do_action( "houzez_property_feed_post_import_properties_domus", $this->import_id );
