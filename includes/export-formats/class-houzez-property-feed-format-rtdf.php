...
class Houzez_Property_Feed_Format_RTDF extends Houzez_Property_Feed_Process {
    ...
    public function send_realtime_feed_request( $post_id ) 
    {
        ...
        foreach ( $exports as $export_id => $export_settings )
        {
            ...
            // ENSURE MAX 25 PROPERTIES, SOMEHOW
    ...
    public function create_send_property_request( $post_id )
    {
        ...
        {
            $request_data['property']['os_status'] = (int)$this->get_mapped_value($post->ID, 'overseas_availability');
        ...
        $request_data['property']['details']['bedrooms'] = ( (get_post_meta( $post->ID, 'fave_property_bedrooms', TRUE ) != '') ? (int)get_post_meta( $post->ID, 'fave_property_bedrooms', TRUE ) : 0 );
        $request_data['property']['details']['bathrooms'] = ( (get_post_meta( $post->ID, 'fave_property_bathrooms', TRUE ) != '') ? (int)get_post_meta( $post->ID, 'fave_property_bathrooms', TRUE ) : null );
        $request_data['property']['details']['reception_rooms'] = ( (get_post_meta( $post->ID, 'fave_property_rooms', TRUE ) != '') ? (int)get_post_meta( $post->ID, 'fave_property_rooms', TRUE ) : null );
        ...
        {
            $virtual_tours[] = array(
                'url' => get_post_meta( $post_id, 'fave_video_url', true ),
                'label' => 'Video'
            );
        }
        ...
        {
            $previous_hash = get_post_meta( $post->ID, '_realtime_sha1_' . $this->export_id, TRUE );
        ...
        if ( $do_request )
        {
            ...
            if ( $request !== FALSE )
            {
                ...
                update_post_meta( $post->ID, '_realtime_sha1_' . $this->export_id, sha1(json_encode($request_data_to_check)) );
            }
        }
        else
        {
    ...
    public function create_remove_property_request( $post_id )
    {
        ...
        if ( $do_request )
        {
            $response = $this->do_curl_request( $request_data, $export_settings['remove_property_url'], $post->ID );
    ...
    public function do_curl_request( $request_data, $api_url, $post_id, $log_success = true ) 
    {
        ...
        $this->log_error("Sending request: " . htmlentities($request_data), '', $post_id);
