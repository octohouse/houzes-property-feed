	...
	<div class="hpf-admin-settings-logs">
		...
		<div class="log-buttons log-buttons-top">
			...
			<?php
				...
				{
					echo ' <a href="' . admin_url( 'admin.php?page=' . ( isset($_GET['page']) ? sanitize_text_field($_GET['page']) : 'houzez-property-feed-import' ) . '&tab=logs&action=view&log_id=' . (int)$previous_instance . ( isset($_GET['import_id']) ? '&import_id=' . (int)$_GET['import_id'] : '' ) ) . '" class="button">Previous Log</a> ';
				}
				if ( $next_instance !== false )
				{
					echo ' <a href="' . admin_url( 'admin.php?page=' . ( isset($_GET['page']) ? sanitize_text_field($_GET['page']) : 'houzez-property-feed-import' ) . '&tab=logs&action=view&log_id=' . (int)$next_instance . ( isset($_GET['import_id']) ? '&import_id=' . (int)$_GET['import_id'] : '' ) ) . '" class="button">Next Log</a> ';
				}
			?>
		</div>
