...
= 2.5.49 - 2026-07-23 =
* Added support for mapping of property features for formats that support it
...
= 2.5.46 - 2026-06-25 =
* Updated Zoopla exports to support multi-currency
* Updated Inmovilla imports to provide additional checking for rental properties
* Updated field rules surrounding core post fields to ensure title, excerpt and content is always retained
* Updated field rules surrounding auto-matching of agent and agency based on name
...
* Added filter 'houzez_property_feed_update_postarr' to override core post data when performing update to existing property
* Updated importing of addresses to ensure they don't contain any duplicate parts
...
* Convert price qualifier in Loop imports from, for example, guidePrice to Guide Price
...
* Import floor area in 10ninety format
* Cater for image URLs with no extension in Property Finder imports
* Corrected PHP errors with Property Finder imports that arose when no price or text-only price was provided
...
* Getrix format to support both an XML and ZIP URL

= 2.5.19 - 2025-05-19 =
* Ensure new JSONPath functionality is only loaded for PHP8.0+. Lower versions threw a fatal error

= 2.5.18 - 2025-05-19 =
* Support for JSONPath syntax when adding field mapping rules to feeds sent in JSON format (i.e. $.attributes.bedrooms)
...
* Added support for mapping contactStaff fields in Contact Information rules from VaultEA/VaultRE
* Corrected error in Kyero exports
...
* Added 'Environment' setting to CASAFARI import format
* Corrected typo in 'Deactivate License Key' button

= 2.5.9 - 2025-03-25 =
* Added new columns in media queue database table to correct issue with media getting duplicated when media queue times out
* Default rent frequency in AgestaNET to 'pcm' for lettings properties
* Corrected issue with agent mapping rules in Kyero imports
...
* Import energy information in Getrix format
...
* Added better logging when error received from ReSales Online API
* Corrected contact info and address mapping not working in Reapit Foundations format
...
* Added new advanced options tab to import settings. One setting at present to limit the number of properties imported
* Corrected EPC PDFs coming in with no extension from 10ninety
...
* Use add_post_meta when setting 'featured' from Pixxi to allow manual change
* Cater for portalAgent field in Pixxi feeds when performing contact mapping

= 2.4.17 - 2024-12-31 =
* Display which import data was imported from above import data on property record. Useful for debugging when multiple imports are in place t ohelp distinguish which import a property originated from
* Correct wrong variable being used in ExpertAgent imports for brochures
* Trim the media captions in Zoopla exports. Spaces would cause rejection

= 2.4.16 - 2024-12-17 =
* Add support for commercial category in Zoopla exports
* Add floorplans to Pixxi imports
* Allow strong tag in import field mapping results
* Tweak to how the separators in numbers are worked out when importing price

= 2.4.15 - 2024-12-12 =
* Catered for Loop changing their image URL formats
* Standardise the image modified date provided by Loop
